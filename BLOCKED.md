...
### stockbot — Lever B HMM configuration not activated + SSH/Jetson connectivity failure (CRITICAL DEADLINE: MAY 22 13:30 UTC — ~50m REMAINING)
**Date blocked**: 2026-05-19 05:10 UTC (Session 1316); SSH auth escalated 2026-05-19 05:39 UTC (Session 1319); Confirmed SSH issue 2026-05-19 08:10 UTC (Session 1324); RE-VERIFIED FAILING 2026-05-19 19:55 UTC (Session 1359); **FINAL ESCALATION 2026-05-22 11:08 UTC (Session 1558)** — SSH STILL FAILING, DEADLINE IMMINENT; **STATUS ESCALATION SESSION 1569 (12:47 UTC)** — JETSON UNREACHABLE, CONNECTION TIMEOUT
**Context**: Pre-checkpoint infrastructure validation (Session 1316) discovered that Lever B HMM regime masking code was deployed to Jetson (`/opt/stockbot/src/ml/hmm_signal_masker.py`), but the config file `/opt/stockbot/config/active-sessions-2session.json` does NOT have `hmm_regime_masking: true` in strategy_params. Result: May 22 checkpoint will execute with Lever A configuration (same as May 19 that already failed with STILL_MISS_B2 outcome), defeating the purpose of Lever B testing. **SSH CONNECTIVITY STATUS (Session 1569, 12:47 UTC)**: Orchestrator connection to ubuntu@100.120.18.84 now fails with "Connection timed out" (vs prior "Permission denied (publickey,password)"). **NEW FAILURE MODE**: Jetson is NOT reachable at all — network unreachable, power off, or Tailscale disconnected. **CRITICAL**: Checkpoint scheduled for 20:00 UTC today (May 22). Cannot verify or fix config without Jetson access. **ESCALATED USER ACTION Deadline: May 22 13:30 UTC (~50m remaining)**.
