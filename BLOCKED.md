...
### stockbot — Engine not running; startup script incompatible with stacker-based trading + Alpaca auth errors
**Date blocked**: 2026-04-29
**Context**: Session 620 verification (2026-04-29 02:01 UTC): (1) Process check confirms engine NOT running (`ps aux | grep run_live_trading` returns empty). (2) Previous "resolution" from Session 615 (in Resolved Archive) falsely claimed engine restarted at 00:16:41 UTC with 11 tickers loaded. Verification reveals engine never started. (3) Recent startup attempt at 2026-04-29 01:06 UTC failed with TWO errors: (a) Script called without required --strategy and --ticker arguments (engine_startup.log shows usage error), (b) Alpaca API returns "401 Authorization Required" errors repeatedly (errors_20260429.log). (4) Script architecture mismatch: `run_live_trading.py` only supports "--strategy buy_and_hold" or "--strategy sma_crossover", but active-sessions.json configures "strategy: stacker:<uuid>" (incompatible format). No built-in mechanism to spawn 11 multi-ticker stacker sessions from active-sessions.json.
**What I need**: (1) Clarify startup mechanism: How should orchestrator invoke the multi-ticker stacker engine? Is there a different entry point, or does run_live_trading.py need a refactor to support "stacker" strategy type? (2) Verify Alpaca API credentials are valid and account has sufficient buying power (auth failures suggest either invalid API key, expired token, or account misconfiguration). (3) Confirm whether active-sessions.json is the intended configuration format, or if a different approach is needed.
**Verify with**: (a) `ps aux | grep run_live_trading | grep -v grep` should show running process; (b) Log file `/home/awank/dev/SuperClaude_Framework/projects/stockbot/logs/trading_20260429.log` should have recent activity (no 401 errors); (c) `curl -H "APCA-API-KEY-ID: <key>" https://api.alpaca.markets/v2/account` should return 200 with account details (not 401).
**Resolution**:
