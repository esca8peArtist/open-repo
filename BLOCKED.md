...
### stockbot — Pre-deployment validation failure: container health check returns sessions=0 (May 28 deployment blocked)

**Date blocked**: 2026-05-27 12:15 UTC (Session 1721 — orchestrator pre-deployment validation)
**Context**: May 28-31 deployment window requires PRE_DEPLOYMENT_VALIDATION_CHECKLIST gates G1-G4 to pass before OPTION_A_RIDGE_WF_RETRAIN_RUNBOOK execution. Gate G3 (Jetson health endpoint) failed: `/health` endpoint returns sessions=0 instead of expected sessions=2 (AAPL baseline sessions). Uvicorn container is running on port 8000 but application appears stuck in stream initialization error loop ('NoneType' object has no attribute 'is_running'). Container logs show repeated warnings: "Stream not fully initialized (market may be closed)" — a known issue when market is closed and realtime stream manager fails to initialize properly. Database files exist and have valid data (1.1M size). Alpaca credentials are set in container environment. JPM lgbm_ho model file present on Jetson (48K). Database backup exists with ~0 trades (file recently created, backup integrity check pending). Other pre-flight gates pass: market hours (MARKET CLOSED — safe), disk space (131GB free), RAM (4576MB available), docker running, config files present.
**What I need**: (1) Diagnose why health endpoint returns sessions=0 (likely the stream manager is blocking session initialization). (2) Determine if this is a transient issue (container needs restart) or a code issue (application stack trace in logs). (3) Decide: proceed with deployment despite health endpoint failure (sessions may initialize once market opens at 13:30 UTC), or resolve stream initialization bug first. (4) Verify database backup integrity (backup file was recently created, may need re-sync from live DB).
**Verify with**: `ssh awank@100.120.18.84 "curl -s http://127.0.0.1:8000/health | python3 -m json.tool | grep sessions"` — should return `"sessions": 2` or higher (expected AAPL 2-session baseline). Also verify: `ssh awank@100.120.18.84 "docker logs stockbot --tail 200 2>&1 | grep -i 'sessions initialized\|session count\|error' | tail -20"` to confirm session initialization status.
**Resolution**: [leave blank — Session 1722 (May 27 12:35 UTC): Attempted verification from orchestrator; SSH connectivity to Jetson unavailable from Pi. User must run verification commands directly on Jetson or from another machine with SSH access. Commands provided above.]
