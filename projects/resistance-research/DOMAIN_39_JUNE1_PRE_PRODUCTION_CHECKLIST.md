---
title: "Domain 39 June 1 Pre-Production Checklist"
created: "2026-05-26"
status: "PRODUCTION-READY — execute May 29 through June 1"
hard_deadline: "June 1, 2026 — HHS interim final rule on OBBBA work requirements"
source_document: "projects/resistance-research/domain-39-healthcare-access-democratic-infrastructure.md"
distribution_strategy: "projects/resistance-research/DOMAIN_39_DISTRIBUTION_STRATEGY.md"
---

# Domain 39 June 1 Pre-Production Checklist
## Healthcare Access as Democratic Infrastructure — HHS Rule Deadline Activation

**Why June 1 matters**: HHS must issue the interim final rule on OBBBA Medicaid work requirement implementation by June 1, 2026. The OBBBA explicitly exempts this rule from public notice and comment. Organizations that have Domain 39's democratic participation argument *before* the rule drops can frame the publication as a democracy event, not just a health policy change. Organizations that receive it after are reacting. This checklist gets you to execution-ready by May 29.

**Scope**: Five pre-production tasks (May 26-29) plus the June 1 send wave. Estimated total time: 3-4 hours across the preparation period.

---

## Step 1 — Source Document Verification

**Deadline**: May 26 (today or within 24 hours of reading this)
**Time**: 20 minutes

Verify the Domain 39 document is current and production-ready before creating the Gist.
...
- [ ] Open the file and confirm the header metadata still shows `status: production-complete`
- [ ] Confirm citation count in the Sources section (count sections: peer-reviewed, government, institutional healthcare, institutional democracy, disability rights, maternal health). Should reach approximately 47.
- [ ] Confirm the Lead Finding section references the June 1 HHS rule as an active deadline — if it says something like "HHS must issue an interim final rule... by June 1, 2026," the document is current.
- [ ] Quick-check the rural hospital closure statistic: 417 vulnerable facilities as of January 2026, referenced in Pathway 1. If you have seen a significantly updated figure in recent news, note it but do not delay the send — include the update in a postscript to your emails.
- [ ] Check one key citation for live access: https://ccf.georgetown.edu/2025/04/17/medicaid-work-reporting-requirements-under-consideration-by-congress-put-people-with-disabilities-cancer-and-those-impacted-by-the-opioid-crisis-at-risk/ — if this returns 404, note it; the citation still exists in the document but you may want to verify the Georgetown CCF site is not experiencing issues before sending to Georgetown CCF directly.

**Outcome**: If the document passes these checks, proceed to Step 2. If the status shows anything other than production-complete, read `DOMAIN_39_DISTRIBUTION_STRATEGY.md` to understand any known gaps before proceeding.

---

## Step 2 — Create the Domain 39 GitHub Gist

**Deadline**: May 29
**Time**: 30-40 minutes

The domain document needs to be publicly accessible via a Gist URL before any emails can be sent. All email templates contain a `[Gist URL — insert before send]` placeholder that you will fill with this URL.

**Steps**:

1. Go to https://gist.github.com
2. Log in with your GitHub account (same account used for Domain 56 Gist: esca8peArtist)
3. Create a new Gist:
   ...
   - **Content**: Paste the full text from `projects/resistance-research/domain-39-healthcare-access-democratic-infrastructure.md`
   - **Visibility**: Public (essential — recipients must be able to open it without a GitHub account)
4. Click "Create public Gist"
5. Copy the Gist URL (format: `https://gist.github.com/esca8peArtist/[hash]`)
6. Test the URL in an incognito browser window — confirm it loads without requiring login, shows the title, and renders markdown correctly

**After creating the Gist, record the URL here**:

Domain 39 Gist URL: _______________________________________________

**Then replace the `[Gist URL — insert before send]` placeholder** in all email templates below (Steps 4 and 5) and in `DOMAIN_39_DISTRIBUTION_STRATEGY.md` (templates section). Use find-and-replace in your text editor.

**Note**: If you do not have a GitHub account or prefer a different hosting method, Notion, HackMD, or a personal website would also work — the requirement is a single stable URL that anyone can open without authentication.

---

## Step 3 — Key Contacts Verification

**Deadline**: May 28
**Time**: 30 minutes

Verify that the 5 highest-priority contact emails and form URLs are still current before the send window. These are the June 1 Tier 1 wave targets.

| # | Organization | Contact Method | URL to Verify |
|---|---|---|---|
| 1 | Georgetown Center for Children and Families | childhealth@georgetown.edu (NOT ccf@georgetown.edu) | https://ccf.georgetown.edu/about-us/contact-us/ |
| 2 | National Health Law Program | info@healthlaw.org OR contact form | https://healthlaw.org/contact/ |
| 3 | Brennan Center for Justice | Contact form | https://www.brennancenter.org/contact |
| 4 | Institute for Responsive Government | Contact form | https://responsivegov.org/contact/ |
| 5 | Black Mamas Matter Alliance | Contact form | https://blackmamasmatter.org/contact/ |

**Verification process** (5 minutes per contact):
- Load the contact URL
- Confirm the page is active (not 404 or parked)
- For email addresses: no verification needed beyond confirming the organization's website still exists
- Note any that have changed their contact process (some nonprofits shift to Submittable or other platforms)

**Contact verification log**:

| Organization | Verified? | Active contact method | Notes |
|---|---|---|---|
| Georgetown CCF | | | |
| NHeLP | | | |
| Brennan Center | | | |
...
## Step 4 — Email Templates: June 1 Tier 1 Wave (Ready to Send)

These are the four highest-priority sends for June 1. Each is a complete, copy-paste-ready email. You need to insert the Gist URL (from Step 2) and your name/contact info into each.

**Two fields to fill in every email**:
- `[YOUR_NAME]` — your real name
- `[YOUR_CONTACT_INFO]` — your personal email address

### Email 1: Georgetown Center for Children and Families

**To**: childhealth@georgetown.edu (CORRECTED — ccf@georgetown.edu is wrong; /contact/ returns 404; use /about-us/contact-us/)
**Timing**: May 30 (two days before the June 1 rule, giving them time to use the analysis)
...
**To**: info@healthlaw.org (or their contact form)
...
**Timing**: June 1 (the day the HHS rule drops — frame the send with the rule as the news hook)
...
**Timing**: June 2-3 (Tier 3 in the distribution strategy — send after Tier 1 responses assessed)
...
## Step 5 — Pre-Send Final Checks (May 31)

Run these checks the day before the June 1 send wave:

- [ ] Gist URL is live (test in incognito window)
- [ ] All 4 Tier 1 emails have `[Gist URL — insert before send]` replaced with the actual URL
- [ ] All 4 Tier 1 emails have `[YOUR_NAME]` and `[YOUR_CONTACT_INFO]` replaced
- [ ] Georgetown CCF email is customized (their April 2025 analysis is referenced — confirm this is still the most recent analysis; check ccf.georgetown.edu for any newer work requirement publications)
- [ ] NHeLP email is customized (their December 2024 / January 2025 analysis is referenced)
- [ ] Brennan Center email includes the current-day HHS rule as the news hook (only appropriate if sending June 1)
- [ ] No new significant court rulings on OBBBA work requirements that require a template update (quick search: "OBBBA Medicaid work requirements injunction" or "work requirements lawsuit 2026")
- [ ] HHS rule has not been delayed or withdrawn (check HHS.gov and healthaffairs.org headlines on May 31)

---

## Execution Calendar

| Date | Action | Time Estimate |
|---|---|---|
| May 26 | Source document verification (Step 1) | 20 min |
| May 27-29 | Create GitHub Gist (Step 2) | 30-40 min |
| May 28 | Contact verification (Step 3) | 30 min |
| May 31 | Insert Gist URL into all email templates (Step 4) | 20 min |
| May 31 | Pre-send final checks (Step 5) | 15 min |
| May 30 | Send Georgetown CCF email (Email 1) | 15 min |
| May 30 | Send NHeLP email (Email 2) | 15 min |
| June 1 | HHS rule drops. Send Brennan Center email (Email 3) | 15 min |
| June 1 | Send Institute for Responsive Government (Email 4) | 15 min |
| June 1 | Log all sends in DISTRIBUTION_EXECUTION_LOG.md | 10 min |
| June 2-3 | Send Black Mamas Matter Alliance (Email 5) | 15 min |
| June 4 | First response check; reply to substantive engagement | 30 min |
| June 5-7 | Congressional staff sends (Senate Finance, House E&C) — per DOMAIN_39_DISTRIBUTION_STRATEGY.md Tier 4 | 45 min |
| June 15 | Mid-campaign check | 20 min |

---

## Send Log (fill as you send)

| # | Organization | Contact | Sent Date | Sent Time | Gist URL Used | Bounce/Error | Notes |
|---|---|---|---|---|---|---|---|
| 1 | Georgetown CCF | childhealth@georgetown.edu | | | | | |
| 2 | NHeLP | info@healthlaw.org | | | | | |
| 3 | Brennan Center | contact form | | | | | |
| 4 | IRG | contact form | | | | | |
| 5 | Black Mamas Matter | contact form | | | | | |

---

## Response Targets

**Minimum viable success (June 1 wave considered effective)**:
- 2 of 4 Tier 1 organizations reply substantively
- At least 1 voting rights organization (Brennan Center or IRG) engages with the NVRA nexus argument
- At least 1 disability rights or reproductive justice organization indicates they will use the analysis

**Escalation trigger**: If Brennan Center or NHeLP signals interest in using Domain 39 for litigation purposes, offer a direct briefing call immediately and accelerate all remaining sends to capture coalition momentum.

---

## Relationship to Domain 56 May 28 Send

Domain 56 (civil service politicization) and Domain 39 (healthcare as democratic infrastructure) are independent send tracks. They do not conflict — organizations in one track are not in the other. The May 28 Domain 56 sends (Volcker Alliance, Democracy Forward, CREW, Government Executive) are separate from the Domain 39 June 1 sends (Georgetown CCF, NHeLP, Brennan Center, IRG, Black Mamas Matter).

If you are executing both on adjacent days:
- May 28: Domain 56 Tier 2 (see `DOMAIN_56_TIER_2_DISTRIBUTION_MAY28_CHECKLIST.md`)
- May 30: Domain 39 first sends (Georgetown CCF, NHeLP)
- June 1: Domain 39 June 1 wave (Brennan Center, IRG)

The only coordination needed: if Democracy Forward replies to the Domain 56 send with PEER v. Trump litigation interest, that is a warm lead for a related conversation — Domain 39's NVRA and APA arguments also support litigation strategy and can be offered as a follow-up document.

---

*Checklist created: May 26, 2026. All email templates are ready for use — insert Gist URL (Step 2) and your name/contact info before sending. Source document: `projects/resistance-research/domain-39-healthcare-access-democratic-infrastructure.md`. Distribution strategy: `projects/resistance-research/DOMAIN_39_DISTRIBUTION_STRATEGY.md`.*
